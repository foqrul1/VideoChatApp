    ...
    private void userInfo(final ViewHolder viewHolder, final String userId, final int pos){
        ...
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                Contacts users = dataSnapshot.getValue(Contacts.class);
                Glide.with(mContext).load(users.getImage()).into(viewHolder.story_photo_seen);
                if(pos != 0){
                    Glide.with(mContext).load(users.getImage()).into(viewHolder.story_photo);
